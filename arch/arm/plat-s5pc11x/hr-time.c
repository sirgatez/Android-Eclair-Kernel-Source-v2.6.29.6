...
//#define HRTIMER_MEASURE_DEBUG
#if defined (HRTIMER_MEASURE_DEBUG)
...
irqreturn_t sec_tick_timer_interrupt(int irq, void *dev_id)
{
	...
	temp = __raw_readl(S5PC1XX_GPH1CON);
	temp = (0x1<<20) ;
	__raw_writel(temp,S5PC1XX_GPH1CON) ;

	temp_data = __raw_readl(S5PC1XX_GPH1DAT);
	temp_data |= (0x1)<<5 ;

	__raw_writel(temp_data,S5PC1XX_GPH1DAT);
	#endif
