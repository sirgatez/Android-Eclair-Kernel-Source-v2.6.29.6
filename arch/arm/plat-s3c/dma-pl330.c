	...
	for (i = 0; i < S3C_CHANNELS_PER_DMA; i++) {
		if (tmp & 0x01) {
			...
			if (buf != NULL) {
				...
				printk(KERN_ERR "dma CH %d: %s: buf %p no buffer in isr\n",
					       chan->number, __FUNCTION__, buf);
			}
			...
			void *dev)
{
	...
	if (!controller->irq_claimed) {
		pr_debug("DMA CH %d: %s : requesting irq %d\n",
			 channel, __FUNCTION__, controller->irq);
...
int s3c2410_dma_ctrl(unsigned int channel, enum s3c_chan_op op)
{
	struct s3c2410_dma_chan *chan = lookup_dma_channel(channel);
			  ...
			  unsigned long devaddr)
{
	...
	case S3C2410_DMASRC_MEM:
		...
		pr_debug("###check point chan->config_flags=%d..\n",chan->config_flags);
			    ...
			    unsigned int stride)
{
	...
	for (controller = 0; controller < S3C_DMA_CONTROLLERS; controller++) {
		...
		if(controller == 0) {
			...
			dma_base = ioremap((S3C_PA_PDMA + ((controller-1) * 0x100000)), stride);
			if (dma_base == NULL) {
	...
	for (channel = 0; channel < channels; channel++) {
		...
		pr_debug("#### Check point..channel= %d and controller= %d\n", channel,controller);
