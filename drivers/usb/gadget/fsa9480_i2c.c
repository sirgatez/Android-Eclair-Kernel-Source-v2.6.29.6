...
{	
		...
		return sprintf(buf, "%s[UsbMenuSel] DEFAULT - NONE\n", buf);	
}
...
{	
	//char *str1 = ""; //SG FIX
	//strcpy(*str1,switchinginitvalue);
	return snprintf(buf, 12, "%d\n", (char *) switchinginitvalue);
}
...
#ifdef FEATURE_TOOL_LAUNCHER
extern void set_tool_launcher_mode(void);
#endif
...
int is_original_setting_modem(void)
{
    ...
    if (sec_get_param_value){
        ...
        return 1; // Return unsuccessful? SG
        }
    usb_sel = switch_sel & (int)(USB_SEL_MASK);
    if(usb_sel == 0)
    {
...
* 2: Done.
*/
void set_tool_launcher_setting_state(int setting_value)
{
