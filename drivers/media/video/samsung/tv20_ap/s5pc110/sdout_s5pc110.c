...
#if 1
//		temp_reg |= SDO_DAC2_Y | SDO_DAC1_Y | SDO_DAC0_Y; 
//		printk("\n temp register +++++ %x \n",temp_reg);
	...
	//2010. 4. 3 chulsang.chang@samsung.com
	//For Regist default coeff
	//#ifdef NON_DEFAULT_OVER_SAMPLING
...
err = gpio_request(GPIO_EAR_SEL,"EAR_SEL");
udelay(50);
gpio_direction_output(GPIO_EAR_SEL,0);
gpio_set_value(GPIO_EAR_SEL,0);
udelay(50);
#else
// Temporary commented for ear-jack detection
/*
gpio_request(GPIO_EARPATH_SEL,"EAR_SEL");
udelay(50);
gpio_direction_output(GPIO_EARPATH_SEL,0);
gpio_set_value(GPIO_EARPATH_SEL,0);
*/
udelay(50);
#endif

err = gpio_request(GPIO_TV_EN,"TV_EN");
udelay(50);
gpio_direction_output(GPIO_TV_EN,1); //!!!
gpio_set_value(GPIO_TV_EN,1);
udelay(50);

#if (defined CONFIG_JUPITER_VER_B4)
err = gpio_request(GPIO_EAR35_SW,"EAR3.5_SW");
udelay(50);
...
//writel(0x300,S5PC11X_GPJ4DRV);

//s3c_gpio_cfgpin(S5PC11X_GPJ4(4), S3C_GPIO_SFN(2));
//s3c_gpio_setpull(S5PC11X_GPJ4(4), S3C_GPIO_PULL_NONE);
