...
#define	ENABLE_FW_RELOAD_FOR_TIMEOUT			1
#define	ENABLE_PROTECT_INTR_TIMEOUT_ERR		1
...
static void mfc_backup_context(mfc_inst_ctx  *mfc_ctx);
static void mfc_restore_context(mfc_inst_ctx  *mfc_ctx);
static void mfc_set_encode_init_param(mfc_inst_ctx *mfc_ctx, mfc_args *args);
static int mfc_get_inst_no(mfc_inst_ctx *mfc_ctx, unsigned int context_addr, int context_size);
static MFC_ERROR_CODE mfc_encode_header(mfc_inst_ctx *mfc_ctx, mfc_args *args);
...
static MFC_ERROR_CODE mfc_alloc_codec_buffer(mfc_inst_ctx *mfc_ctx, mfc_args *args);
static MFC_ERROR_CODE mfc_alloc_dec_frame_buffer(mfc_inst_ctx *mfc_ctx, mfc_args *args);
static MFC_ERROR_CODE mfc_alloc_context_buffer(mfc_inst_ctx *mfc_ctx, unsigned int mapped_addr, unsigned int *context_addr, int *size);
static MFC_ERROR_CODE mfc_alloc_stream_ref_buffer(mfc_inst_ctx *mfc_ctx, mfc_args *args);
...
//static int CheckIProfileError(unsigned char *src_mem, unsigned int streamSize);

static int mfc_mem_inst_no[MFC_MAX_INSTANCE_NUM];
static bool mCheckType = false;
...
*/
#if	DEBUG_MAKE_RAW
static int mframe_cnt = 0;
static int mImgWidth = 0;
static int mImgHight= 0;
...
static bool mfc_cmd_host2risc(mfc_facade_cmd cmd, int arg1, int arg2, int arg3, int arg4)
{
	mfc_facade_cmd cur_cmd = 0;
...
{
	/*
	memcpy(mfc_ctx->MfcSfr, mfc_sfr_base_vaddr, MFC_REG_SIZE);
	*/
}
...
{
	/*
	memcpy(mfc_sfr_base_vaddr, mfc_ctx->MfcSfr, MFC_REG_SIZE);
	*/
}
...
static void mfc_set_dec_stream_buffer(mfc_inst_ctx *mfc_ctx, int buf_addr, unsigned int buf_size)
{
	...
	/* 2009.08.21 by icarus : release buffer */
...
static void mfc_set_dec_frame_buffer(mfc_inst_ctx *mfc_ctx)
{
	...
	mfc_frame_buf_arg_t dpb_buff_addr;
	unsigned int luma_plane_sz, chroma_plane_sz, mv_plane_sz;
...
{
	unsigned int port0_base_paddr, port1_base_paddr, i;
	mfc_strm_ref_buf_arg_t ref_buf_addr;
	unsigned int luma_plane_sz, chroma_plane_sz;
	mfc_enc_init_mpeg4_arg_t *init_arg;
...
static void mfc_set_encode_init_param(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	mfc_enc_init_mpeg4_arg_t *enc_init_mpeg4_arg;
	mfc_enc_init_h264_arg_t *enc_init_h264_arg;

	enc_init_mpeg4_arg = (mfc_enc_init_mpeg4_arg_t *)args;
	enc_init_h264_arg = (mfc_enc_init_h264_arg_t *)args;
...
MFC_FW_RESET:
	...
	if (mfc_wait_for_done(R2H_CMD_SYS_INIT_RET) != R2H_CMD_SYS_INIT_RET)
	{
		 ...
		 //if( READL(0x58) == 0x00)
		{
			...
			{
				mfc_err("RETRY : MFCINST_ERR_FW_LOAD_FAIL\n");
				return MFCINST_ERR_FW_LOAD_FAIL;				
			}
...
static unsigned int mfc_get_codec_arg(SSBSIP_MFC_CODEC_TYPE codec_type)
{
...
static MFC_ERROR_CODE mfc_encode_one_frame(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	mfc_enc_exe_arg *enc_arg;
	unsigned int port0_base_paddr, port1_base_paddr;
	int	interrupt_flag;
	int 	nReturnErrCode;
	

	enc_arg = (mfc_enc_exe_arg *)args;
...
MFC_ERROR_CODE mfc_exe_encode(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	//	ret_code = mfc_encode_one_frame(mfc_ctx, enc_arg);
...
MFC_ERROR_CODE mfc_init_decode(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	 ...
	 */
	//WRITEL(mfc_ctx->postEnable, MFC_LF_CONTROL);
	...
	{
		WRITEL(mfc_ctx->widthDivX311, MFC_SI_CH0_DIVX311_HRESOL);
		WRITEL(mfc_ctx->heightDivX311, MFC_SI_CH0_DIVX311_VRESOL);
	}
	...
	if (mfc_ctx->MfcCodecType == DIVX311_DEC)
	{
		mfc_ctx->img_width = mfc_ctx->widthDivX311;
		mfc_ctx->img_height = mfc_ctx->heightDivX311;

		init_arg->out_img_width = mfc_ctx->widthDivX311;
		init_arg->out_img_height =  mfc_ctx->heightDivX311;
		init_arg->out_buf_width = ALIGN_TO_128B(mfc_ctx->widthDivX311);
		init_arg->out_buf_height = ALIGN_TO_32B(mfc_ctx->heightDivX311);
	}
...
static MFC_ERROR_CODE mfc_decode_one_frame(mfc_inst_ctx *mfc_ctx, mfc_dec_exe_arg_t *dec_arg, unsigned int *consumed_strm_size)
{
	...
	{
		/* 2009.08.20 by icarus : address shift */
	...
	mfc_ctx->FrameType = (mfc_frame_type)(frame_type & 0x3);
...
{
	mfc_get_config_arg_t *get_cnf_arg;
	get_cnf_arg = (mfc_get_config_arg_t *)args;
...
{
	mfc_set_config_arg_t *set_cnf_arg;
	set_cnf_arg = (mfc_set_config_arg_t *)args;
	...
	switch (set_cnf_arg->in_config_param)
	{
		...
		case MFC_DEC_SETCONF_DIVX311_WIDTH_HEIGHT:
			...
			mfc_ctx->widthDivX311 = set_cnf_arg->in_config_value[0];
			mfc_ctx->heightDivX311 = set_cnf_arg->in_config_value[1];
			break;
