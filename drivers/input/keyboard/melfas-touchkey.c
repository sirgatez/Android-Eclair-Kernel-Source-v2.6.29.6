...
void  touchkey_work_func(struct work_struct * p)
{
    ...
    if(!gpio_get_value(_3_GPIO_TOUCH_INT))
    {
        ...
        if((data[0] & ESD_STATE_BIT) ||(ret !=0))
        {
            ...
            //releae key 
            input_report_key(touchkey_driver->input_dev, KEY_BACK, 0);
            input_report_key(touchkey_driver->input_dev, KEY_MENU, 0);
            retry = 10;
            while(retry--)
            {
...
static int __init touchkey_init(void)
{
	...
	/* F/W Ver 10h & Module Ver 99 is TEST firmware*/
	if( (FW_VER == 0x10) && (MODULE_VER == 99 || MODULE_VER == 0xFF) )
	{
		bNeedToUpdateFirmware = 1;
	}
	/* The current last firmware info is 
		F/W Ver 6
		Module Ver 10
	 */
	else if( (FW_VER < 8) || (FW_VER == 0xff) )
	{
		bNeedToUpdateFirmware = 1;
	}
	
	if( bNeedToUpdateFirmware == 1 )
	{
		bNeedToUpdateFirmware = 0;
